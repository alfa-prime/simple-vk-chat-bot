class Messages:
    ...
    def info():
        return f'' \
               f'1. По команде "Поиск" бот запрашивает для кого искать пару:\n\n' \
               ...
               f'2. Если у пользователя есть "избранные" варианты в "белом" списке (смотри пункт 8), то ' \
               f'предлагается просмотреть их, с возможностью удаления кандидатов из "белого" списка\n\n' \
               f'3. Бот запрашивает пол кандитатов, пользователь сам выбрает (мужской, женский);\n\n' \
               f'4. Бот запрашивает семейное положение кандидатов, пользователь сам выбирает ' \
               f'(в активном поиске или неженатые);\n\n' \
               f'5. Если возраст пользователя известен, предлагается выбор:\n' \
               f'- ровестники (возраст пользователя +/- 2 года);\n' \
               f'- возрастной диапазон;\n\n' \
               f'в противном случае, только возрастной диапазон.\n\n' \
               f'7. Если город пользователя известен, предлагается выбор:\n' \
               f'- искать в родном городе\n' \
               f'- указать другой город\n\n' \
               f'в противном случае, пользователь сам указывает город\n\n' \
               f'8. При просмотре результатов поиска, есть возможность добавлять кандидатуру ' \
               f'в "белый" или "черный" списки (кнопки "Да" и "Нет" соотвественно), кнопка "Не знаю" ' \
               f'просто пролистывает кадидатов дальше. Кандидаты добавленные в "белый" или "черный" ' \
               f'списки при последующем поиске не показываются'
